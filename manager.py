...
class Manager:
    ...
    def destroy(self, j: "index child PCB"):
        ...
        for k in child_proc.children.copy():
            child_proc.children.remove(k)
            self.ready_list.remove(k)
            self._PCB_list.free(k)
    ...
    def timeout(self):
        ...
        self._set_running_proc(running_proc_list[0])
        self.scheduler()
